...
| **№** | **Требование**                                                                                                     |
|:-----:|:-------------------------------------------------------------------------------------------------------------------|
|  NFR1   | Актуальность данных: ставки должны обновляться в системах кол-центра не позднее чем через 30 минут после изменения |
|  NFR2   | Доступность систем кол-центра для консультаций 24/7                                                                |
|  NFR3   | Время отклика API для получения ставок не более 2 секунд                                                           |
|  NFR4  | Размер файла со ставками не должен превышать 1 МБ                                                                  |
|  NFR5 | Безопасность: передача ставок должна осуществляться по защищенным каналам связи                                    |
|  NFR6 | Совместимость с существующей системой кол-центра (Java / Spring Boot)                                              |
|  NFR7 | Использование SFTP-протокола для партнерского кол-центра (нет возможности API-интеграции)                          |
|  NFR8 | Минимизация изменений в существующих системах                                                                      |
|  NFR9 | Возможность масштабирования на несколько партнерских кол-центров                                                   |
...
#### [Диаграмма контекста](https://github.com/AlexanderMATI/architecture-pro-standart/blob/main/Task4/Task4%20Context.puml)
...
#### [Диаграмма контейнеров](https://github.com/AlexanderMATI/architecture-pro-standart/blob/main/Task4/Task4%20Container.puml)
...
- Двойная интеграция: для внутреннего кол-центра используется REST API (соответствует требованию NFR6 о совместимости с Java Spring Boot), для партнерского кол-центра - SFTP файловый обмен (соответствует ограничению NFR7).
- Автоматизация обновления: ставки автоматически обновляются в базе данных сервиса при получении новых данных от бэк-офиса, что обеспечивает актуальность данных (требование NFR1).
- Безопасность: Все интеграции используют защищенные протоколы - HTTPS для API и SFTP для файлового обмена (требование NFR5).
...
|  T5   | Интеграция с Rates API                                              | Система кол-центра (внутренняя) |
|  T6  | Настройка SFTP-инфраструктуры                                       | SFTP-сервер                     |
|  T7  | Мониторинг и администрирование                                      | SFTP-сервер                     |
|  T8  | Разработка клиента для SFTP                                         | Система партнерского кол-центра |
